$(document).ready(function() {
	...
	Crafty.load(["images/sprite.png", "images/bg.png", "images/car.png"], function() {
...
function initPlayer(){
  var player = Crafty.e("2D, canvas, image, collision");
	player.attr({x: Crafty.viewport.width / 2, y: Crafty.viewport.height / 2, w:29, h:95});
	player.origin("center");
	player.image("images/sprite.png","no-repeat");
  ...
  player.bind("keydown", function(e) {
	...
	player.collision();
	player.onhit("car",function(){
	  
	  alert("Boom! (great job, champ, time to go take your 'random' drug screening)");
	  Crafty.scene("main");
	});
...
Crafty.c("car",function(){
  
});

function build_car(attrs){
  var car = Crafty.e("2D, canvas, image, collision, car");
  car.attr({w:29,h:54});
  car.attr(attrs);
  car.image("images/car.png","no-repeat");
  car.draw();
  //car.collision();
  return car;
}
...
function initCars(){
  build_car({x:150,y:320,rotation: 90});
  build_car({x:500,y:270,rotation: 270});
  build_car({x:940,y:500,rotation: 0});
  build_car({x:938,y:560,rotation: 0});
  build_car({x:280,y:100,rotation: 180});
}
